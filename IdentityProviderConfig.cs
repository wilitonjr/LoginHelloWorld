...
namespace SQLSafe.Login.Poc
{
    ...
    internal class IdentityProviderConfig
    {
        ...
        private static readonly string ENTRA_PROFILE_URL = "https://graph.microsoft.com/v1.0/me";
        ...
        private static readonly string REDIRECT_LOGOUT_CALLBACK = "http://localhost:5000/logout-callback";
            ...
            switch (provider)
            {
                case IdentityProvider.Okta:
                    return $"{GetAuthority(provider)}/v2/logout?" +
                        $"client_id={Uri.EscapeDataString(GetClientId(provider))}" +
                        $"&returnTo={Uri.EscapeDataString(REDIRECT_LOGOUT_CALLBACK)}";
                case IdentityProvider.EntraID:
                    return $"{GetAuthority(provider)}/logout?" +
                        $"&post_logout_redirect_uri={Uri.EscapeDataString(REDIRECT_LOGOUT_CALLBACK)}";
        ...
        public static string GetUserInfoUrl(IdentityProvider provider)
        {
            switch (provider)
            {
                ...
                case IdentityProvider.EntraID:
                    return ENTRA_PROFILE_URL;
                default:
                    throw new ArgumentException("Provider not found");
            }
        }

        public static string GetLoginRedirectUri(IdentityProvider provider)
        {
            return REDIRECT_URI;
        }
